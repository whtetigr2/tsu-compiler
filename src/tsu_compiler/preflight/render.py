...
This module follows `tsu_compiler.report`'s discipline (see that module's docstring):
...
equivalent of `tsu_compiler.report._as_unavailable`.
"""
...
def _as_unavailable(text: str) -> str:
    """Mirrors `tsu_compiler.report._as_unavailable`: every stand-in for a missing
...
def _format_band(band) -> str:
    ...
    within the sweep -- see `tsu_compiler.preflight.sweep.usable_band`'s own
...
def _fmt_opt(value, reason: str, fmt: str) -> str:
    ...
    `RegimeRow.abs_m_err`/`tau`/`n_eff` are None exactly when tsu_compiler.ess judged
...
def write_preflight(report, out_dir) -> list[Path]:
    ...
    if getattr(report, "coupling_note", None):
        lines.append("")
        lines.append("> **Coupling parameters:** " + report.coupling_note)
...
def write_regime(rows, band, cross, out_dir, *, onsager,
    ...
    never checked at all -- `tsu_compiler.preflight.sweep.detect_uniform_square_
    lattice` is what a caller should use to get a real three-state answer;
    `tsu_compiler.cli.sweep_single_model` is the one caller in this codebase that
    does.
    ...
    lines.append("\n`tau` here is tau_A = 1 + 2 * sum_k rho_k (Sokal/emcee "
                 ...
                 "not a real property -- see `tsu_compiler.preflight.sweep.sweep`).")
    lines.append("\nRows marked **provisional** have R-hat above "
                 ...
                 "tsu_compiler.ess judged tau/ESS itself too noisy to quantify an "
