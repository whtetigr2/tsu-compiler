...
from tsu_compiler.passes.place import _try_grid_embed
...
"""tsu_compiler.ess's own wording (see `effective_sample_size` in ess.py) for the
...
def detect_uniform_square_lattice(ising: IsingModel) -> tuple[float | None, str]:
    ...
    `tsuc regime` run and then rendered that as "not applicable to this
...
class RegimeRow:
    ...
    `abs_m_err`, `tau` and `n_eff` are None together whenever tsu_compiler.ess judged the
    ...
    one merged flag, because they mean different things:
      ...
      - `ess_unavailable` (tsu_compiler.ess judged N/tau too low): only the tau
    ...
    when the first attempt already cleared tsu_compiler.ess's reliability floor, and
...
def sweep(model_fn, sizes, couplings, *, seed: int = 0, n_chains: int = 16,
    ...
    `tsu_compiler.ess.RELIABILITY_MIN_N_OVER_TAU=5000` even once tau exceeds 1 by a
    ...
    repeatedly until either the estimate clears tsu_compiler.ess's reliability floor
    ...
    for size in sizes:
        for bj in couplings:
            ...
            # reason from tsu_compiler.ess already says exactly why, unqualified.
            ...
            # N_eff = N_total/tau. tau_A = 1 + 2*sum_k rho_k (tsu_compiler.ess's own
            ...
            # Deliberately NOT done inside tsu_compiler.ess itself -- that module is
