...
def _try(spec, target, encoding, allow_assumed, clamp=None, coefficient_scale=1.0,
    ...
    (via the receipt's program.json) a later `tsuc simulate` replaying it.
    """
...
def _measure_mixing(ising, chains: np.ndarray) -> EssEstimate:
    ...
    tsu_compiler.ess is the sample's own energy under `ising`, the standard physics-
...
# above, `tsu_compiler.ess.RELIABILITY_MIN_N_OVER_TAU`'s reliability floor -- recorded
...
# not a bug in this function or in `tsu_compiler.ess`. Deliberately NOT changed here:
...
def _energy_scale(spec, enc, ising) -> tuple[float | None, str]:
    ...
    that only `src/tsu_compiler/backends/` may import it) reports None with a reason,
