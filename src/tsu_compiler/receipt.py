...
def _env():
    # Nothing outside src/tsu_compiler/backends/ may import thrml directly; obtain its
...
def write_receipt(c, out_dir) -> Path:
    ...
    # G1 (FORMULATION layer of `tsuc explain`): WHY each spec construct
    ...
    passes = {"verdict": c.verdict, "ideal_passed": c.ideal_passed,
              ...
              # read back later, e.g. by `tsuc report`/`tsuc explain` or by a
    ...
    # C3 (ENERGY layer of `tsuc explain`): the term inventory of the ENCODED
    ...
    # -- so `tsuc explain` can state the energy model's shape without
    ...
    # G2 (final APPLICATION layer of `tsuc explain`): ONE concrete decoded
    ...
    if c.program is not None:
        ...
        prog = {"form": c.program.form,
                ...
                # spins (spec 5.3) -- persisted verbatim so `tsuc simulate`'s
