...
execute it. See src/tsu_compiler/rules.py for the module docstring on why four of the
twelve `RULE_CLASSES` read as domain vocabulary while remaining pure data.
"""


def test_rule_carries_its_class_and_provenance():
    from tsu_compiler.rules import Rule, load_rules
    rules = load_rules([{
...
def test_unknown_rule_class_is_rejected_not_silently_accepted():
    ...
    exists to prevent."""
    import pytest
    from tsu_compiler.rules import load_rules
